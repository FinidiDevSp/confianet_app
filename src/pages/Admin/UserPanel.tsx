...
import React, { useEffect, useState } from 'react';
import { Alert, Button, Card, CardBody, Col, Container, Input, Label, Modal, ModalBody, ModalFooter, ModalHeader, Row, Spinner, Table } from 'reactstrap';
import axios from 'axios';
import { getLoggedinUser, setAuthorization } from '../../helpers/api_helper';
...
const UserPanel: React.FC = () => {
  ...
  const [delegations, setDelegations] = useState<any[]>([]); // org-wide (admin real)
  const [myDelegations, setMyDelegations] = useState<any[]>([]); // for header
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const meRes: any = await axios.get('/api/auth/me');
        setMe(meRes);
        // My active delegations for header
        try {
          const mine: any = await axios.get('/api/admin/delegations/me/active');
          setMyDelegations(mine as any);
        } catch {}
        // Organization delegations only for real admin
        if (meRes.role === 'admin') {
          try {
            const del: any = await axios.get('/api/admin/delegations');
            setDelegations(del as any);
  ...
  const createDelegation = async () => {
    try {
      ...
      if (!expiresAt) { setError('Indica fecha/hora de expiración'); return; }
      const payload = { grantee_email: granteeEmail, roles: [roles[0]], expires_at: new Date(expiresAt).toISOString() };
      await axios.post('/api/admin/delegations', payload);
      setMessage('Delegación creada');
      const del: any = await axios.get('/api/admin/delegations');
      setDelegations(del as any);
    } catch (e:any) { setError(e?.message || 'No se pudo crear la delegación'); }
